...
def populate_occupations_with_csv():
    with open('occupations_links.csv') as csvfile:
        reader = csv.reader(csvfile)
        rows = list(reader) # [['Job Class', 'Percentage', 'Link'], ['Management', '6.1', 'hjttps://'], ...]
        rows = rows[1:-1] # remove first and last row

    for row in rows:
        occupations[row[0]] = float(row[1])
        occupations[row[0]] = row[2]
...
def hello_world():
    ...
    new_line = '\n'
    occulink = occupations[occupation][1]
    content = f"TNPG: Tic-Tac, Talia Hsia, Ayman Habib, Craig Chen <br> <br> <link href=occulink>{occupation} <br> <br> From list: {occupations}"
    return(content)
